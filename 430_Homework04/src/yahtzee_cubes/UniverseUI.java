...
public class UniverseUI extends JPanel {
    ...
    /**
     * Character representing unknown cube state, displayed
     * for cubes with value 0 (no neighbors).
     */
     ...
     * @param value
     */
    public void updateDisplay(final int index, final int value) {
        Runnable r = new Runnable() {
            @Override
            public void run() {
                boxes[index].setText(value);
                repaint();
            }
        };
    ...
    public int getNeighbor(final int index, final boolean left) {
        return findNeighbor(index, left);
    }
    ...
    private int findNeighbor(int index, boolean left) {
        ...
        for (int i = 0; i < boxes.length; ++i) {
            if (i != index) {
                if (Math.abs(target - boxes[i].x) <= HORIZONTAL_MAX && Math.abs(current.y - boxes[i].y) <= VERTICAL_MAX) {
                    return i;
                }
            }
        }
